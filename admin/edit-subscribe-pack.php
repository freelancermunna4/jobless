<?php
  ...
  if($id==0){
    header('location:add-subscribe-pack');
    exit;
  }
  if(isset($_POST['submit'])){
    ...
    $insert=_insertData($db,"UPDATE `youtube_sub_packs` SET `name`='$pname',`coins`='$coin' WHERE id=$id");
    header('location:add-subscribe-pack');
  ...
  $uid=$data['id'];
  $pac=_getData($db,"SELECT * FROM youtube_sub_packs WHERE id=$id AND activity=1");
