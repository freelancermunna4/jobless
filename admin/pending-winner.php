<?php
  ...
  $users=_getAllData($db,"SELECT * FROM louckywinner WHERE activity=0 ORDER BY id ASC LIMIT $start,25");
                ...
                <div class="overflow-auto bg-white">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed">
                        <thead class="bg-white">
                            <tr>
                                <th scope="col"
                                    class="p-4 text-xs font-medium text-left text-gray-500 uppercase lg:p-5"> Coupon
                                    Code </th>
                                <th scope="col"
                                    class="p-4 text-xs font-medium text-left text-gray-500 uppercase lg:p-5"> Price
                                </th>
                                <th scope="col"
                                    class="p-4 text-xs font-medium text-left text-gray-500 uppercase lg:p-5"> Activity
                                </th>
                                <th scope="col"
                                    class="p-4 text-xs font-medium text-left text-gray-500 uppercase lg:p-5"> User
                                    Name </th>
                                <th scope="col"
                    ...
                    foreach($users as $c){?>
                            ...
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="p-4 space-x-2 whitespace-nowrap lg:p-5">
                                    <button type="button" onclick="couponaprove(<?= $c['id'] ?>)"
                                        class="popup_show btn bg-green-600 w-fit text-white">Aproved</button>
                                    <button data-target="delete_coupon" type="button" onclick="userid(<?= $c['id'] ?>)"
                    ...
                    <div class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
                        ...
                        <div
                          ...
                          $users=_getAllData($db,"SELECT * FROM louckywinner WHERE activity=0 ORDER BY id ASC");
                          echo count($users);
                           ?></span>
...
function couponaprove(data) {
    ...
    $.ajax({
        ...
        success: function(z) {
            if (z.trim() == "success") {
                swal("Success!", "Successfully Aproved", "success");
                location.reload(true);
            } else {
                swal(z);
                $('#apv').show();
