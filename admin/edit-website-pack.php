<?php
  ...
  if($id==0){
    header('location:add-website-packs');
    exit;
  }
  if(isset($_POST['submit'])){
    ...
    $insert=_insertData($db,"UPDATE `web_surfing_pckks` SET `name`='$pname',`coins`='$coin',`time`='$tim' WHERE id=$id");
  ...
  $uid=$data['id'];
  $pac=_getData($db,"SELECT * FROM web_surfing_pckks WHERE id=$id");
