...
$uid=$data['id'];
  $video=_getAllData($db,"SELECT * FROM vad_videos WHERE status=1 ORDER BY id DESC LIMIT $start,25");
                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed">
                        <thead class="bg-white">
                            <tr>
                                ...
                                <th scope="col"
                                    class="p-4 text-xs font-medium text-left text-gray-500 uppercase lg:p-5"> Video
                                    Title
                                </th>
                                <th scope="col"
                            ...
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap lg:p-5">
                                    <a href="https://www.youtube.com/watch?v=<?= $v['video_id'] ?>" target="_blank"
                                        style="color: blue;"><?= $v['video_id'] ?></a>
                                </td>
                                <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap lg:p-5">
                                    <?= $v['today_clicks'] ?> today - <?= $v['visits'] ?> total
                                </td>
                                <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap lg:p-5">
                                    <?= $v['clickneed'] ?> x <?= $v['tim'] ?> seconds </td>
                                <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap lg:p-5">
                                ...
                                <td class="p-4 space-x-2 whitespace-nowrap lg:p-5">
                                    <a href="./edit-video.php?id=<?= $v['id'] ?>" type="button"
                                        class="btn bg-green-600 w-fit text-white">Edit</a>
                                    <button onclick="getid(<?= $v['id'] ?>)" data-target="delete_video" type="button"
                    ...
                    <div class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
                        ...
                        <div
                            ...
                            <div class="w-fit">
                             ...
                             $video=_getAllData($db,"SELECT * FROM vad_videos WHERE status=1 ORDER BY id DESC");
                             echo count($video);
                              ?></span>
                            ...
                            " class="relative inline-flex items-center rounded-l-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-500 hover:bg-gray-50 focus:z-20">
                                    ...
                                    <a href="active-videos?strt=<?php if($totaluser>$start){ echo ($start+25);
                            }else{
                              echo $start;
...
<div data-target="delete_video"
    ...
    <div class="relative px-4 w-full max-w-md h-full md:h-auto z-50">
        <div class="relative bg-white rounded-2xl shadow-lg">
            <div class="flex justify-end p-2"><button type="button" data-target="delete_video"
            ...
            <div class="p-6 pt-0 text-center text-5xl text-red-500"> <i class="fa-solid fa-circle-exclamation"></i>
                <h3 class="my-9 text-base font-normal text-gray-500">Are You Sure, Want to delete this video?
                </h3>
                <div class="w-full flex justify-between items-center gap-x-2"><a href="#" data-target="delete_video"
