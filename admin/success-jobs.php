...
$uid=$data['id'];
  $jobs=_getAllData($db,"SELECT * FROM job_submit WHERE activity=1 ORDER BY id DESC LIMIT $start,25");
...
<div class="x_container space-y-10 py-10">
    ...
    <div class="flex flex-col rounded-lg shadow-md border border-[
                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed">
                        <thead class="bg-white">
                            <tr>
                                ...
                                <th scope="col"
                                    class="p-4 text-xs font-medium text-left text-gray-500 uppercase lg:p-5">New Link
                                </th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="p-4 text-sm font-normal text-gray-500 lg:p-5">
                                    <div class="max-w-[300px]"><?= $j['title'] ?></div>
                                </td>
                                ...
                                <td class="p-4 space-x-2 whitespace-nowrap lg:p-5">
                                    <a target="_blank" href="<?= $j['oldUrl'] ?>"
                                        class="text-blue-600 hover:underline"><?= substr($j['oldUrl'], 0, 25); ?>...
                                    </a>
                                </td>
                                <td class="p-4 space-x-2 whitespace-nowrap lg:p-5">
                                    <a target="_blank" href="<?= $j['submiturl'] ?>"
                                        class="text-blue-600 hover:underline"><?=  substr($j['submiturl'], 0, 25); ?>...
                                    </a>
                                </td>
                            </tr>
                            ...
                            <div class="w-fit">
                                <p class="text-sm text-gray-700">
                                    ...
                                    <span class="font-medium"><?php
                                     $jobs=_getAllData($db,"SELECT * FROM job_submit WHERE activity=1 ORDER BY id ASC");
                                     echo count($jobs);
                                      ?></span>
      ...
      " class="relative inline-flex items-center rounded-l-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-500 hover:bg-gray-50 focus:z-20">
                                    ...
                                    <a href="success-jobs?strt=<?php if($totaluser>$start){ echo ($start+25);
      }else{
        echo $start;
